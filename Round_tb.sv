...
interface R_if (input bit clk);
  ...
  covergroup cg @(posedge clk);
    ...
    cp2: coverpoint key_valid_in;
    cp3: coverpoint reset;
    cp4: coverpoint data_in {option.auto_bin_max = 512;}
    cp5: coverpoint round_key {option.auto_bin_max = 512;}
  endgroup
  cg cg_inst = new();
  ...
  clocking ck @(posedge clk);
    ...
    output reset, data_valid_in, data_in, key_valid_in, round_key;
  endclocking
...
class transaction;
  rand bit [127:0]data_in, round_key;
  rand bit data_valid_in, key_valid_in;
  bit reset;
  bit [127:0]data_out;
  bit valid_out;
  ...
  function void display(input string tag, bit reset=0);
    $display("[%s] :  reset = %0b \t data_valid_in = %0b \t key_valid_in = %0b \t data_in = %32x \t round_key = %32x \tvalid_out = %0b \t dout = %32x", tag, reset, data_valid_in, key_valid_in, data_in, round_key, valid_out, data_out);
  endfunction
...
class driver;
  ...
  task run();
    forever begin
      mbx.get(datac);
      datac.display("DRV");
      rif.data_in = datac.data_in;
      rif.data_valid_in <= datac.data_valid_in;
      rif.key_valid_in <= datac.key_valid_in;
      rif.round_key <= datac.round_key;
      repeat(8) @(posedge rif.clk);
    end
  endtask
...
class monitor;
  ...
  task run();
    ...
    forever begin
      ...
      tr.reset = rif.reset;
      tr.data_valid_in = rif.data_valid_in;
      tr.key_valid_in = rif.key_valid_in;
      tr.data_out = rif.data_out;
      tr.data_in = rif.data_in;
...
class scoreboard;
  ...
  event next;
  int i;
  bit [127:0]doutSB, doutSR, doutMC, dout;
  bit [7:0]State[16];
  ...
  task func(input [127:0]din, input [127:0]round_key);
  ...
  task run();
    forever begin
      mbx.get(tr);
      tr.display("SCO");
      func(tr.data_in, tr.round_key);
      $display("%32x", dout);
      if(!tr.reset && tr.data_out == 0 && tr.valid_out == 0) $display("Data Match");
      else if (tr.reset && tr.data_out == dout && tr.valid_out == tr.data_valid_in&tr.key_valid_in) $display("Data Match");
...
module tb();
  ...
  R_if rif(clk);
  Round dut(rif.clk, rif.reset, rif.data_valid_in, rif.key_valid_in, rif.data_in, rif.round_key, rif.valid_out, rif.data_out);
